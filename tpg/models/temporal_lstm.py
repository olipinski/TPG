...
from tpg.models.base_lstm import BaseLSTMNetwork
from tpg.utils import gumbel_softmax_sample


class TemporalLSTMNetwork(BaseLSTMNetwork):
    """The Temporal LSTM Network class."""

    def __init__(self, *args, **kwargs):
        """
        Network for analysing the understanding of temporal progression in emergent communication.

        Will use the same arguments as BaseLSTM Network.
        """
        ...
        # Do they repeat?
        self.sender_temporal_lstm = nn.LSTM(
        ...
        self.receiver_temporal_lstm = nn.LSTM(
    ...
    def infer(
        ...
        # So the meaning LSTM processes 128 sequences of length 1.
        # Below we change to [1,128,4]. So the LSTM processes one continuous sequence
        # to understand the relationships within.
        embedding_meaning_temporal, _ = self.sender_temporal_lstm(
        ...
        message_temporal, _ = self.receiver_temporal_lstm(message_decoded)
